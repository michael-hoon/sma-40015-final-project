...
export const SERVED_TYPES = ['medication'];

export default class RobotMEDi extends Agent {
  /**
   * @param {object} params
  ...
  /**
   * Decision phase (tick step 2). Only called when IDLE with sufficient battery.
   */
  decideAndClaim() {
  ...
  _beginServing(currentTick) {
    ...
    const [min, max] = this.config.SERVICE_TIME.robot[needType];
  ...
  _goCharge() {
    ...
    // Nearest charging bay by Manhattan distance
    let nearest = bays[0];
    ...
    const reached = this.setDestination(nearest);
    if (reached) {
      this.state = STATES.MOVING_TO_CHARGER;
    }
  }
